...
class _CenteredTapePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < lanes.length; i++) {
      ...
      if (laneLabel.isNotEmpty && bandSpans.isNotEmpty) {
        ...
        for (final s in bandSpans) {
          final leftV = max(s.leftX, left) + 4; // a little inner margin
          final rightV = min(s.rightX, right) - 4;
    ...
    for (int mH = firstMidnight; mH <= endTick + 24; mH += 24) {
      ...
      final segLeft = min(x0, x1);
      final segRight = max(x0, x1);
      ...
      final visL = max(segLeft, left) + 4;
      final visR = min(segRight, right) - 4;
