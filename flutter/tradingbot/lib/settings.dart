...
class _ApiSettingsCardState extends State<_ApiSettingsCard> {
  ...
  bool _loading = false; // render immediately; fetch in background
  bool _saving = false;
  bool _testing = false;
  ...
  Future<void> _load() async {
    ...
    try {
      final r = await http
          .get(Uri.parse('/api/openai/settings'))
      ...
      // ignore — keep UI usable; user can still paste keys and Save/Test
    } finally {
  ...
  Future<void> _test() async {
    ...
    try {
      // Allow test if either a key is already saved OR user has typed a new one.
      final hasInlineKey =
          _openaiKeyC.text.isNotEmpty && _openaiKeyC.text != _MASK;
      if (!_hasOpenAIKey && !hasInlineKey) {
        messenger?.showSnackBar(const SnackBar(
            content: Text('Enter your OpenAI key or save settings first.')));
        return;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Padding(
        padding: const EdgeInsets.all(16),
        // Always render the form; background sync just tweaks fields if available.
        child: Form(
          key: _formKey,
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      ...
                      onChanged: (v) => setState(() => _model = v ?? 'gpt-5'),
                  ...
                  Row(
                    children: [
                      ...
                      Switch.adaptive(
                        ...
                        onChanged: (v) => setState(() => _enableBrowsing = v),
                      ),
                    ],
                  ),
                ],
                              ...
                              : () async {
                                  ...
                                  try {
                                    final r = await http.post(
                                      ...
                                      body: jsonEncode({"openai_api_key": ""}),
                                    );
                                    if (r.statusCode >= 200 &&
                                        r.statusCode < 300) {
                                      setState(() {
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText:
                      _hasSearchKey ? 'Search API key (set)' : 'Search API key',
                              ...
                              : () async {
                                  ...
                                  try {
                                    final r = await http.post(
                                      ...
                                      body: jsonEncode({"search_api_key": ""}),
                                    );
                                    if (r.statusCode >= 200 &&
                                        r.statusCode < 300) {
                                      setState(() {
